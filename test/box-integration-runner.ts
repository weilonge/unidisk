/**
 * Box FUSE integration test runner — executed as a standalone Node.js process.
 *
 * Usage:
 *   npx tsx test/box-integration-runner.ts --token <token>
 *   BOX_TOKEN=<token> npx tsx test/box-integration-runner.ts
 *
 ...
 *
 * Setup / teardown:
 *   A uniquely-named test folder is created in the Box root before the FUSE
 *   mount and deleted after unmount, regardless of test outcome.  No other
 *   account data is modified.
 *
 * Because real Box account contents are unknown, file tests are adaptive:
...
import { Box } from '../src/clouddrive/Box'
...
function resolveToken(): string | null {
  ...
  return process.env.BOX_TOKEN ?? null
}
...
async function runTests(mountPoint: string, testFolderName: string): Promise<void> {
  ...
  console.log('\nErrno behaviour')
  try {
    await fs.readFile(path.join(mountPoint, '__box_integration_nonexistent_file_xyz__.txt'))
...
async function main(): Promise<void> {
  ...
  if (!token) {
    console.log('Box integration test skipped — no --token provided.')
    console.log('Supply one via: --token <token>  or  BOX_TOKEN=<token>')
    process.exit(0)
  }

  const MOUNT_POINT      = path.join(os.tmpdir(), 'ud-box-fuse-test-mnt')
  const TEST_FOLDER      = `/__ud_test_${Date.now()}`
  const TEST_FOLDER_NAME = path.basename(TEST_FOLDER)

  const provider = new Box()
  ...
  console.log(`Mounting Box at ${MOUNT_POINT} …`)
