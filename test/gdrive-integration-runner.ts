/**
 * Google Drive FUSE integration test runner — executed as a standalone Node.js process.
 *
 * Usage:
 *   npx tsx test/gdrive-integration-runner.ts --token <token>
 *   GDRIVE_TOKEN=<token> npx tsx test/gdrive-integration-runner.ts
 *
 ...
 *
 * Setup / teardown:
 *   A uniquely-named test folder is created in the Drive root before the FUSE
...
import { GoogleDrive } from '../src/clouddrive/GoogleDrive'
...
function resolveToken(): string | null {
  ...
  return process.env.GDRIVE_TOKEN ?? null
}
...
async function runTests(mountPoint: string, testFolderName: string): Promise<void> {
  console.log('\nGoogle Drive FUSE Integration Tests')
  console.log('=====================================')
  ...
  console.log('\nErrno behaviour')
  try {
    await fs.readFile(path.join(mountPoint, '__gdrive_integration_nonexistent_file_xyz__.txt'))
...
async function main(): Promise<void> {
  ...
  if (!token) {
    console.log('Google Drive integration test skipped — no --token provided.')
    console.log('Supply via: --token <token>  or  GDRIVE_TOKEN=<token>')
    console.log('Get a token: https://developers.google.com/oauthplayground')
    console.log('  → select "Drive API v3 → https://www.googleapis.com/auth/drive"')
    process.exit(0)
  }

  const MOUNT_POINT      = path.join(os.tmpdir(), 'ud-gdrive-fuse-test-mnt')
  const TEST_FOLDER      = `/__ud_test_${Date.now()}`
  const TEST_FOLDER_NAME = path.basename(TEST_FOLDER)

  const provider = new GoogleDrive()
  ...
  console.log(`Mounting Google Drive at ${MOUNT_POINT} …`)
