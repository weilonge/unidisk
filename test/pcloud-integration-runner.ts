/**
 * pCloud FUSE integration test runner — executed as a standalone Node.js process.
 *
 * Usage:
 *   npx tsx test/pcloud-integration-runner.ts --token <token>
 *   PCLOUD_TOKEN=<token> npx tsx test/pcloud-integration-runner.ts
 *
 * EU-region accounts:
 *   PCLOUD_TOKEN=<token> PCLOUD_HOST=eapi.pcloud.com npx tsx test/pcloud-integration-runner.ts
 *
 ...
 *
 * Setup / teardown:
 *   A uniquely-named test folder is created in the pCloud root before the FUSE
 *   mount and deleted after unmount, regardless of test outcome.  No other
...
import { pCloud } from '../src/clouddrive/pCloud'
...
function resolveToken(): string | null {
  ...
  return process.env.PCLOUD_TOKEN ?? null
}
...
async function runTests(mountPoint: string, testFolderName: string): Promise<void> {
  ...
  console.log('\nErrno behaviour')
  try {
    await fs.readFile(path.join(mountPoint, '__pcloud_integration_nonexistent_file_xyz__.txt'))
...
async function main(): Promise<void> {
  ...
  if (!token) {
    console.log('pCloud integration test skipped — no --token provided.')
    console.log('Supply via: --token <token>  or  PCLOUD_TOKEN=<token>')
    console.log('EU accounts: also set PCLOUD_HOST=eapi.pcloud.com')
    process.exit(0)
  }

  const apiHost = process.env.PCLOUD_HOST ?? 'api.pcloud.com'

  const MOUNT_POINT      = path.join(os.tmpdir(), 'ud-pcloud-fuse-test-mnt')
  const TEST_FOLDER      = `/__ud_test_${Date.now()}`
  const TEST_FOLDER_NAME = path.basename(TEST_FOLDER)

  const provider = new pCloud()
  provider.init({ token, apiHost, cacheStore: 'memory' })

  const udm = new UdManager()
  udm.init({
    provider,
    profile:             { token, apiHost, cacheStore: 'memory' },
  ...
  console.log(`Mounting pCloud at ${MOUNT_POINT} …`)
